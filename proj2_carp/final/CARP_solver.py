...
def merge_split(sx: Solution, p) -> Solution:
    routes = sx.routes.copy()
    free = []
    candidate_routes = []
    final_cost = sx.total_cost
    for i in range(0, p):
        index = random.randint(0, len(routes) - 1)
        candidate_routes.append(routes[index])
        final_cost -= routes[index].cost
        routes.pop(index)
        if len(routes) == 1:
            break
    for route in candidate_routes:
        for edge in route.path:
            free.append(edge)
            free.append((edge[1], edge[0]))
    best_cost = float('inf')
    best_routes = None
    best_load = None
    for rule in rules:

        routes, total_cost, total_load = path_scanning(free.copy(), routes.copy())
    ...
    for route in best_routes:
        routes.append(route)
    return Solution(best_routes, final_cost + best_cost, sx.total_load)
...
def local_search(sx: Solution) -> Solution:
    ...
    # solution = merge_split(solutions[0], 2)
    # if solution.total_cost < solutions[0].total_cost:
